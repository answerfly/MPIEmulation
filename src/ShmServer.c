...
int main(int argc,char **argv)
{
	...
	shmid_msg = create_shm(1024,msgid);
	...
	switch(rmr->MsgType)
	{
		case Send:
			...
			free(buf);
		 	break;
		case Recv:			
			printf("message type is recv,typesize is %d, recvcount is %d,totaltime is %ld\n",rmr->typesize,rmr->recvcount,rmr->time);
		...
		case Isend:
			...
			break;
		case Issend:
			//printf("message type is Issend\n");
			r=ProcessIssend(rmr,IssendTable);
			free(buf);
			break;
		case Iprobe:
			printf("message type is Iprobe, flag = %d\n", rmr->recvcount);
...
int ProcessShmRequest(ShmMReq shmmreq)
{
	...
	switch(shmmreq.MsgType)
	{
		...
		case Recv:			
            ...
            if((msgcws->totalsize - msg->shmmreq.head) < BUFFER_SIZE)
			{
		...
		case Alltoall:
            ...
            if((msgcwos->totalsize - msg->shmmreq.head) < BUFFER_SIZE)
			{
