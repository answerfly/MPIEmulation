...
int myIfContain(MyHashMap * const map, void * key,
		int(*equal)(void *key1, void *key2)) {
	...
	while (myHashMapEntryIteratorHasNext(it)) {
		...
		int * aa = (int *) entry->key;
		int * bb = (int *) entry->value;
		//printf("LF: key address is %d, value address is %d\n", aa,bb);
		//printf("LF: key is %d, value is %d\n", *aa,*bb);
		if (entry->key == key || (equal != NULL && (*equal)(entry->key, key))) {
			return 1;
		}
	}
